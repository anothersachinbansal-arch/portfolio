...
async function addBooksToProduction() {
  try {
    ...
    console.log(`📚 Found ${existingBooks.length} existing books in production database`);

    if (existingBooks.length > 0) {
      console.log("Existing books:");
      existingBooks.forEach((book, index) => {
        console.log(`${index + 1}. ${book.title} (ID: ${book.bookId})`);
      });
    }
    ...
    for (const bookData of originalBooks) {
      ...
      if (!existingBook) {
        ...
        console.log(`⚠️ Book already exists: ${existingBook.title} (ID: ${existingBook.bookId})`);
      }
    }

    // Final check
    const finalBooks = await Book.find({});
    console.log(`🎉 Total books in production database: ${finalBooks.length}`);
